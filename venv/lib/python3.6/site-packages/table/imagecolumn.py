...
class ImageColumn(Column):
    ...
    def render(self, obj):
        ...
        template = Template('{%% load static %%}<img style="width: 100px; height: 150px;" src="%s"'
                            ' title="%s">' % (path, title))
        return template.render(Context())

class ImageColumn2(Column):
    def __init__(self, field=None, image_title=None, *args, **kwargs):
        kwargs["sortable"] = False
        kwargs["searchable"] = False
        self.image_title = image_title
        super(ImageColumn2, self).__init__(field=field, *args, **kwargs)
    ...
    def render(self, obj):
        ...
        if isinstance(self.image_title, Accessor):
            ...
            title = self.image_title
        template = Template('{%% load static %%}<img style="width: 175px;" src="%s"'
                            ' title="%s">' % (path, title))
        return template.render(Context())
